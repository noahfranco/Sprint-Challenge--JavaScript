...
console.log(tyrannosaurus1.roarMethod);
...
const universities = [];
graduates.forEach((Element, index) => {universities.push(Element.university)});  
...
const contactInfo = [];
graduates.forEach((Element, index) => {contactInfo.push(`${Element.first_name} ${Element.email}`)})
...
graduates.forEach((Element) => {uni.toString(`${uni.university}`)}) 
...
zooAnimals.map((Element, index) => {lowerCase.push(`${Element.animal_name}`)})
...
zooAnimals.filter((Element) => {lowerPopulation.push(Element.population < 5, Element.animal_name)}) 
